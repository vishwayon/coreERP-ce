...
namespace app\core\ac\bankReco;
...
class ModelBankReco {
    ...
    function GetRecoData() {
        ...
        $cmm->setCommandText('select * from ac.sp_bank_reco_collection(:pcompany_id, :pbranch_id, :paccount_id, :preconciled, :pfrom_date, :pas_on)');
        ...
        foreach ($recodt->Rows() as &$dr) {
            ...
            $dr['cheque_date'] = ['display' => \app\cwf\vsla\utils\FormatHelper::FormatDateForDisplay($dr['cheque_date']),
                'sort' => strtotime($dr['cheque_date'])];
        }
        $this->dt = $recodt;
    ...
    public function setData($model) {
        ...
        if (count($this->brokenrules) == 0) {
            ...
            try {
                ...
                $cmm = new \app\cwf\vsla\data\SqlCommand();
                $cmm->setCommandText('select * from ac.sp_bank_reco_update(:ptype, :pvoucher_id, :paccount_id, :pcollected, :pcollection_date)');
                $cmm->addParam('ptype', 'A');
                $cmm->addParam('pvoucher_id', '');
                $cmm->addParam('paccount_id', $model->account_id);
                $cmm->addParam('pcollected', false);
                $cmm->addParam('pcollection_date', null);
                $cn->beginTransaction();
                for ($rowIndex = 0; $rowIndex < count($model->dt); $rowIndex++) {
                    if ($model->view_type_id == self::VIEW_TYPE_RECONCILED) {
                        if ($model->dt[$rowIndex]->collected == FALSE) {
                            $cmm->setParamValue('ptype', $model->dt[$rowIndex]->reco_type);
                            $cmm->setParamValue('pvoucher_id', $model->dt[$rowIndex]->voucher_id);
                            $cmm->setParamValue('pcollected', 0);
                            $cmm->setParamValue('pcollection_date', Null);
                            DataConnect::exeCmm($cmm, $cn);
                        }
                    }
                    if ($model->view_type_id == self::VIEW_TYPE_UNRECONCILED) {
                        if ($model->dt[$rowIndex]->collected == TRUE) {
                            $cmm->setParamValue('ptype', $model->dt[$rowIndex]->reco_type);
                            $cmm->setParamValue('pvoucher_id', $model->dt[$rowIndex]->voucher_id);
                            $cmm->setParamValue('pcollected', 1);
                            if ($model->dt[$rowIndex]->collection_date == '1970-01-01') {
                                $cmm->setParamValue('pcollection_date', date("Y-m-d", time()));
                            } else {
                                $cmm->setParamValue('pcollection_date', $model->dt[$rowIndex]->collection_date);
                            }
                            DataConnect::exeCmm($cmm, $cn);
                        }
                        if ($model->dt[$rowIndex]->collected == FALSE) {
    ...
    public function validate($model) {
        for ($rowIndex = 0; $rowIndex < count($model->dt); $rowIndex++) {
            if ($model->view_type_id == self::VIEW_TYPE_UNRECONCILED) {
                if ($model->dt[$rowIndex]->collected == TRUE) {
                    if ($model->dt[$rowIndex]->collection_date == '1970-01-01') {
                        array_push($this->brokenrules, $model->dt[$rowIndex]->voucher_id . ' : Select proper Collection Date.');
                    }
                    if (strtotime($model->dt[$rowIndex]->collection_date) <
                            strtotime(\app\cwf\vsla\utils\FormatHelper::GetDBDate($model->dt[$rowIndex]->doc_date->display))) {
                        array_push($this->brokenrules, $model->dt[$rowIndex]->voucher_id . ' : Collection Date cannot be less than Doc Date');
                    }
                }
            }
        }
