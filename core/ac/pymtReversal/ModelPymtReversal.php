...
class ModelPymtReversal {
    ...
    public function setFilters($filter) {
        ...
        $this->find_vch_id = trim($filter['find_vch_id']);
        if (strpos($this->find_vch_id, 'PYMT') !== 0 && strpos($this->find_vch_id, 'ASP') !== 0 &&
                strpos($this->find_vch_id, 'BPV') !== 0 && strpos($this->find_vch_id, 'PAYB') !== 0) {
            $this->brokenrules[] = 'Reversal entries are allowed only for Supplier Payment, Advanced Supplier Payment, Bank Payment';
        } else {
            if (strpos($this->find_vch_id, 'PYMT') === 0 || strpos($this->find_vch_id, 'ASP') === 0) {
                $this->category = 'A';
            } else if (strpos($this->find_vch_id, 'BPV') === 0) {
                $this->category = 'B';
            }
            $this->getData();
        }
    ...
    public function getData() {
        ...
        $cmm->setCommandText('select * from ac.sp_pymt_reversal_collection(:pcompany_id, 0, :pvoucher_id, :paccount_id)');
        ...
        if (count($this->dtVch->Rows()) > 0) {
            if ($this->dtVch->Rows()[0]['vchstatus'] == 'OK') {
                ...
                $this->supp_name = $this->dtVch->Rows()[0]['supplier'];
    ...
    public function setData($model) {
        if (strpos($model->find_vch_id, 'PYMT') === 0 || strpos($model->find_vch_id, 'ASP') === 0) {
            $model->category = 'A';
        } else if (strpos($model->find_vch_id, 'BPV') === 0 ) {
            $model->category = 'B';
        }
        $this->validate($model);
        if (count($this->brokenrules) == 0) {
            ...
            try {
                $cn->beginTransaction();
                $cmm = new \app\cwf\vsla\data\SqlCommand();
                $cmm->setCommandText('select * from ac.sp_pymt_reversal_update(:pcompany_id, 0, :pvoucher_id, :paccount_id, :preversal_date, :preversal_remark, :pcategory, :pfinyear)');
                $cmm->addParam('pcompany_id', SessionManager::getInstance()->getUserInfo()->getSessionVariable('company_id'));
    ...
    public function validate($model) {
        if (strpos($model->find_vch_id, 'PYMT') !== 0 && strpos($model->find_vch_id, 'ASP') !== 0 &&
                strpos($model->find_vch_id, 'BPV') !== 0 ) {
            $this->brokenrules[] = 'Reversal entries are allowed only for Supplier Payment, Advanced Supplier Payment, Bank Payment';
        }
        if ($model->reversal_date == NULL || $model->rev_remark == NULL) {
            $this->brokenrules[] = 'Reversal date and remarks are required.';
        } else {
        ...
        if (strpos($model->doc_id, 'ASP') === 0) {
            $cmm = new \app\cwf\vsla\data\SqlCommand();
            $cmm->setCommandText('select * from ac.general_ledger where voucher_id = :pvoucher_id||\':TDS\'');
            $cmm->addParam('pvoucher_id', $model->doc_id);
            $rdt = DataConnect::getData($cmm);
            if (count($rdt->Rows()) > 0) {
                $this->brokenrules[] = ' Voucher has TDS entry. Only non TDS advances can be reversed.';
            }
    ...
    private function logAction($model) {
        ...
        $cmm->addParam('putil', 'Payment Reversal');
