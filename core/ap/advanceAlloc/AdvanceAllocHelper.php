...
namespace app\core\ap\advanceAlloc;
...
class AdvanceAllocHelper {
    ...
    public static function CreateAllocTemp($bo) {
        ...
        $bo->payable_ledger_temp = new \app\cwf\vsla\data\DataTable();
        ...
        $scale = 0;
        $isUnique = false;
        $bo->payable_ledger_temp->addColumn('branch_id', \app\cwf\vsla\data\DataAdapter::getDBtoPHPDataType('int8'), $default, 0, $scale, $isUnique);
        $bo->payable_ledger_temp->addColumn('account_id', \app\cwf\vsla\data\DataAdapter::getDBtoPHPDataType('int8'), $default, 0, $scale, $isUnique);
        $bo->payable_ledger_temp->addColumn('fc_type_id', \app\cwf\vsla\data\DataAdapter::getDBtoPHPDataType('int8'), $default, 0, $scale, $isUnique);


        $phpType = \app\cwf\vsla\data\DataAdapter::getDBtoPHPDataType('varchar');
        $default = \app\cwf\vsla\data\DataAdapter::getPHPDataTypeDefault($phpType);
        $bo->payable_ledger_temp->addColumn('voucher_id', $phpType, $default, 50, $scale, $isUnique);
        $bo->payable_ledger_temp->addColumn('vch_tran_id', $phpType, $default, 50, $scale, $isUnique);
        $bo->payable_ledger_temp->addColumn('rl_pl_id', $phpType, $default, 500, $scale, $isUnique);
        $bo->payable_ledger_temp->addColumn('narration', $phpType, $default, 500, $scale, $isUnique);
        $bo->payable_ledger_temp->addColumn('bill_no', $phpType, $default, 50, $scale, $isUnique);

        $phpType = \app\cwf\vsla\data\DataAdapter::getDBtoPHPDataType('numeric');
        $default = \app\cwf\vsla\data\DataAdapter::getPHPDataTypeDefault($phpType);
        $bo->payable_ledger_temp->addColumn('exch_rate', $phpType, $default, 0, 6, $isUnique);
        $bo->payable_ledger_temp->addColumn('balance', $phpType, $default, 0, 4, $isUnique);
        $bo->payable_ledger_temp->addColumn('balance_fc', $phpType, $default, 0, 4, $isUnique);
        $bo->payable_ledger_temp->addColumn('credit_amt', $phpType, $default, 0, 4, $isUnique);
        $bo->payable_ledger_temp->addColumn('credit_amt_fc', $phpType, $default, 0, 4, $isUnique);

        $phpType = \app\cwf\vsla\data\DataAdapter::getDBtoPHPDataType('date');
        $default = \app\cwf\vsla\data\DataAdapter::getPHPDataTypeDefault($phpType);
        $bo->payable_ledger_temp->addColumn('doc_date', $phpType, $default, 0, $scale, $isUnique);
        $bo->payable_ledger_temp->addColumn('bill_date', $phpType, $default, 0, $scale, $isUnique);

        $phpType = \app\cwf\vsla\data\DataAdapter::getDBtoPHPDataType('bool');
        $default = \app\cwf\vsla\data\DataAdapter::getPHPDataTypeDefault($phpType);
        $bo->payable_ledger_temp->addColumn('is_select', $phpType, $default, 0, $scale, $isUnique);

        foreach ($bo->payable_ledger_temp->getColumns() as $col) {
            $cols[] = ['columnName' => $col->columnName, 'default' => $col->default];
        }
        $bo->setTranMetaData('payable_ledger_temp', $cols);
    }

    public static function ValidateAdvance($bo, $account_id, $voucher_id) {
        if (count($bo->payable_ledger_alloc_tran->Rows()) > 0) {
            if ($account_id != $bo->payable_ledger_alloc_tran->Rows()[0]['account_id']) {
                $bo->addBRule('Advance settlement does not belong to the selected supplier. Kindly resettle advances.');
            }
        }
        $date_changed = false;
        foreach ($bo->payable_ledger_alloc_tran->Rows() as $row) {
        ...
        foreach ($bo->payable_ledger_alloc_tran->Rows() as &$refpl_alloc_row) {
            if ($bo->fc_type_id == 0) {
                $refpl_alloc_row['credit_amt_fc'] = 0;
            } else {
                $refpl_alloc_row['credit_amt'] = round($refpl_alloc_row['credit_amt_fc'] * $bo->exch_rate, \app\cwf\vsla\Math::$amtScale);
            }
        ...
        // Validate Excess advances if any
        if ($bo->fc_type_id == 0) {
            ...
            $current_alloc = $bo->payable_ledger_alloc_tran->select(['rl_pl_id', 'credit_amt']);
    ...
    public static function GetAdvAllocDetailsOnEdit($bo, $voucher_id) {
        $cmm = new \app\cwf\vsla\data\SqlCommand();
        $cmm->setCommandText('Select a.rl_pl_id, a.voucher_id, a.bill_no, a.bill_date, a.doc_date, b.debit_amt, b.debit_amt_fc
                                from ac.rl_pl a
                                inner join ac.rl_pl_alloc b on a.rl_pl_id = b.rl_pl_id
                                Where b.voucher_id = :pvoucher_id');
        $cmm->addParam('pvoucher_id', $voucher_id);
        $resultTemplate = \app\cwf\vsla\data\DataConnect::getData($cmm);
        foreach ($bo->payable_ledger_alloc_tran->Rows() as &$refplrow) {
            foreach ($resultTemplate->Rows() as $row) {
                if ($refplrow['rl_pl_id'] == $row['rl_pl_id']) {
                    $refplrow['adv_ref_id'] = $row['voucher_id'];
                    $refplrow['doc_date'] = $row['doc_date'];
                    $refplrow['adv_ref_date'] = $row['doc_date'];
                    $refplrow['bill_date'] = $row['bill_date'];
                    $refplrow['bill_no'] = $row['bill_no'];
                }
            }
        }
    }

    public static function GetUnsettledAdvAmt($supplier_id, $doc_date) {        
        $cmm = new \app\cwf\vsla\data\SqlCommand();
        $cmm->setCommandText('select COALESCE(sum(balance), 0.00) as balance from ap.fn_payable_ledger_balance(:pcompany_id, :pbranch_id, :paccount_id, :pto_date, :pvoucher_id, :pdc)');
