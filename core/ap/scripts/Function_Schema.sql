...
BEGIN	
    ...
    where (a.account_id=psupplier_id or psupplier_id=0)
            ...
            And status = 5
    Union All
    Select a.voucher_id, a.doc_date, b.vch_tran_id, b.sl_no, (a.annex_info->>'purchase_account_id')::bigint, c.account_head, b.item_amt, 0, 
            e.material_name, COALESCE((f.annex_info->'satutory_details'->>'gst_state_id')::bigint, -1), 
            COALESCE((f.annex_info->'satutory_details'->>'gstin')::varchar, ''), 
            g.address || E'\n' || g.city || case when g.pin = '' then '' else ' - ' end  
                        || g.pin || case when g.state = '' then '' else E'\n' end  || g.state || case when g.country = '' then '' else E'\n' end || g.country, 
            COALESCE((a.annex_info->'gst_input_info'->>'vat_type_id')::bigint, -1), 
            (d.sgst_amt + d.cgst_amt + d.igst_amt)
    from md.doc_control a
    inner join md.doc_tran b on a.voucher_id = b.voucher_id
    inner join ac.account_head c on (a.annex_info->>'purchase_account_id')::bigint = c.account_id
    inner join tx.gst_tax_tran d on b.vch_tran_id = d.gst_tax_tran_id
    inner join st.material e on b.material_id = e.material_id
    inner join ap.supplier f on b.supplier_id = f.supplier_id
    inner join sys.address g on f.address_id = g.address_id
    where (b.supplier_id = psupplier_id or psupplier_id=0)
            And a.branch_id = pbranch_id
            And case when pvoucher_id = '' then a.doc_date between pfrom_date and pto_date 
    ...
    if exists (SELECT * FROM information_schema.tables where table_schema='pub' And table_name = 'abp_control') then		
        insert into bill_temp(bill_id, doc_date, bill_tran_id, sl_no, account_id, account_head, bill_amt, bill_amt_fc, 
                    ...
                    supp_addr, vat_type_id,
                    tax_amt)
        select a.voucher_id, a.doc_date, b.vch_tran_id, b.sl_no, a.payable_account_id, e.account_head, b.gross_amt, b.gross_amt_fc, 
                c.edition_desc, COALESCE((a.annex_info->'gst_input_info'->>'supplier_state_id')::bigint, -1), 
                COALESCE((a.annex_info->'gst_input_info'->>'supplier_gstin')::varchar, ''), 		 
                COALESCE((a.annex_info->'gst_input_info'->>'supplier_address')::varchar, ''), a.vat_type_id,
                (f.sgst_amt + f.cgst_amt + f.igst_amt)
        from pub.abp_control a
        inner join pub.abp_tran b on a.voucher_id = b.voucher_id
        inner join pub.abp_adv_tran c on b.voucher_id = c.voucher_id and b.ro_tran_id = c.ro_tran_id
        inner join pub.ro_control d on b.ro_id = d.ro_id
        inner join ac.account_head e on a.payable_account_id = e.account_id
        inner join tx.gst_tax_tran f on b.vch_tran_id = f.gst_tax_tran_id
        where (a.account_id=psupplier_id or psupplier_id=0)
                And a.branch_id=pbranch_id
                And case when pvoucher_id = '' then a.doc_date between pfrom_date and pto_date 
                        else a.voucher_id = pvoucher_id 
                ...
                And a.status = 5
        Union All 
        select a.voucher_id, a.doc_date, b.vch_tran_id, b.sl_no, a.payable_account_id, f.account_head, g.gross_amt, g.gross_amt_fc, 
                c.radio_location_desc || ' - ' || d.radio_time_band_name, COALESCE((a.annex_info->'gst_input_info'->>'supplier_state_id')::bigint, -1), 
                COALESCE((a.annex_info->'gst_input_info'->>'supplier_gstin')::varchar, ''), 		 
                COALESCE((a.annex_info->'gst_input_info'->>'supplier_address')::varchar, ''), a.vat_type_id,
                (h.sgst_amt + h.cgst_amt + h.igst_amt)
        from pub.abp_control a
        inner join pub.abp_tran g on a.voucher_id = g.voucher_id
        inner join pub.abp_radio_tran b on g.voucher_id = b.voucher_id and g.ro_tran_id = b.ro_tran_id
        ...
        inner join ac.account_head f on a.payable_account_id = f.account_id
        inner join tx.gst_tax_tran h on b.vch_tran_id = h.gst_tax_tran_id
        where (a.account_id=psupplier_id or psupplier_id=0)
                And a.branch_id=pbranch_id
                And case when pvoucher_id = '' then a.doc_date between pfrom_date and pto_date 
                        else a.voucher_id = pvoucher_id 
        ...
        Union All 
        select a.voucher_id, a.doc_date, b.vch_tran_id, b.sl_no, a.payable_account_id, f.account_head, g.gross_amt, g.gross_amt_fc, 
                c.tv_location_desc || ' - ' || d.tv_time_band_name, COALESCE((a.annex_info->'gst_input_info'->>'supplier_state_id')::bigint, -1), 
                COALESCE((a.annex_info->'gst_input_info'->>'supplier_gstin')::varchar, ''), 
                COALESCE((a.annex_info->'gst_input_info'->>'supplier_address')::varchar, ''), a.vat_type_id,
                (h.sgst_amt + h.cgst_amt + h.igst_amt)
        from pub.abp_control a
        inner join pub.abp_tran g on a.voucher_id = g.voucher_id
        inner join pub.abp_tv_tran b on g.voucher_id = b.voucher_id and g.ro_tran_id = b.ro_tran_id
        ...
        Union All 
        select a.voucher_id, a.doc_date, b.vch_tran_id, b.sl_no, a.payable_account_id, f.account_head, g.gross_amt, g.gross_amt_fc, 
                c.web_location_desc, COALESCE((a.annex_info->'gst_input_info'->>'supplier_state_id')::bigint, -1), 
                COALESCE((a.annex_info->'gst_input_info'->>'supplier_gstin')::varchar, ''), 
                COALESCE((a.annex_info->'gst_input_info'->>'supplier_address')::varchar, ''), a.vat_type_id,
                (h.sgst_amt + h.cgst_amt + h.igst_amt)
        from pub.abp_control a
        inner join pub.abp_tran g on a.voucher_id = g.voucher_id
        inner join pub.abp_web_tran b on g.voucher_id = b.voucher_id and g.ro_tran_id = b.ro_tran_id
