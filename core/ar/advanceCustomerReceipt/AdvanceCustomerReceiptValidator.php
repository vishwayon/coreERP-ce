...
/**
 * Description of AdvanceCustomerReceiptValidator
 *
 * @author Priyanka
 */
class AdvanceCustomerReceiptValidator extends  \app\cwf\vsla\xmlbo\ValidatorBase {
    
    public function validateAdvanceCustomerReceiptEditForm() {
    ...
    public function validateBusinessRules() { 
        ...
        if($this->bo->fc_type_id ==0){
            $this->bo->debit_amt_fc=0;
        }
        else{
            $this->bo->debit_amt=round(($this->bo->debit_amt_fc * $this->bo->exch_rate), \app\cwf\vsla\Math::$amtScale);
        }
        $currency='';
        $subCurrency='';
        $currency_system='';
        ...
        if($this->bo->fc_type_id == 0){
            if($this->bo->debit_amt == 0 && $this->bo->tds_amt == 0){
                $this->bo->addBRule('Gross received cannot be nil. Enter amount received/tax withheld.'); 
            }
        }
        else{  
            if($this->bo->debit_amt_fc == 0){
                $this->bo->addBRule('Gross Received FC is required.'); 
            }
        }
        ...
        // Gst validations
        if($this->bo->annex_info->Value()->gst_ref->is_mat == false 
                && $this->bo->annex_info->Value()->gst_ref->is_income == false 
                && floatval($this->bo->debit_amt) > 1000) {
            $this->bo->addBRule('Select either material or income for GST when gross amount is greater than INR 1,000/-');
        }
        if($this->bo->annex_info->Value()->gst_ref->is_mat == true 
                && $this->bo->annex_info->Value()->gst_ref->is_income == true) {
            $this->bo->addBRule('Select either material or income for GST. Both not allowed');
        }
        if($this->bo->annex_info->Value()->gst_ref->is_mat == true 
                && $this->bo->annex_info->Value()->gst_ref->material_id == -1) {
            $this->bo->addBRule('Select material/service for GST');
        } 
        if($this->bo->annex_info->Value()->gst_ref->is_mat == false) {
            $this->bo->annex_info->Value()->gst_ref->material_type_id = -1;
            $this->bo->annex_info->Value()->gst_ref->material_id = -1;
        }
        
        if($this->bo->annex_info->Value()->gst_ref->is_income == true 
                && $this->bo->annex_info->Value()->gst_ref->income_account_id == -1) {
            $this->bo->addBRule('Select Income Account/Type for GST');
        }
        if($this->bo->annex_info->Value()->gst_ref->is_income == false) {
            $this->bo->annex_info->Value()->gst_ref->income_type_id = -1;
            $this->bo->annex_info->Value()->gst_ref->income_account_id = -1;
        }
