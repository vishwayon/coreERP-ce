 ...
 * @author Priyanka
 */
class CustManualSetEventHandler extends \app\cwf\vsla\xmlbo\EventHandlerBase {

    public function afterFetch($criteriaparam) {
        parent::afterFetch($criteriaparam);
        $this->bo->trigger_id = 'core';
        if ($this->bo->voucher_id == "" or $this->bo->voucher_id == "-1") {
            $this->bo->customer_id = $criteriaparam['formData']['SelectCustomer']['account_id'];
            ...
            if (count($criteriaparam['formData']['SelectVch']) > 0) {
                ...
                $this->bo->invoice_id = $criteriaparam['formData']['SelectVch'][0]['voucher_id'];
                ...
                $this->bo->rl_id = $criteriaparam['formData']['SelectVch'][0]['rl_pl_id'];
            } else {
    ...
    public function onFetch($criteriaparam, $tablename) {
        ...
        if ($tablename == 'ar.customer') {
            $ac = \app\cwf\vsla\entity\EntityManager::getInstance()->getActionScripts('ar.customer', \app\cwf\vsla\data\DataConnect::COMPANY_DB, \app\cwf\vsla\entity\ActionScript::TABLE_TYPE_MASTER_CONTROL);
            $cmm = new \app\cwf\vsla\data\SqlCommand();
            $cmm = $ac->getFetchCmm();
            $cmm->setParamValue('pcustomer_id', $criteriaparam['formData']['SelectCustomer']['account_id']);
            $dt = \app\cwf\vsla\data\DataConnect::getData($cmm);
            if (count($dt->Rows()) > 0) {
                $this->bo->customer = $dt->Rows()[0]['customer'];
            }
        }
    }
    ...
    public function onSave($cn, $tablename) {
        ...
        if ($tablename == 'ar.customer') {
            ...
            // To delete existsing allocations for save ACR
            ...
            $cmmDel->addParam('pvoucher_id', $this->bo->invoice_id);
            $cmmDel->addParam('paccount_id', $this->bo->customer_id);
            $cmmDel->addParam('prl_pl_id', $this->bo->rl_id);
            foreach ($this->bo->receivable_ledger_alloc_tran->Rows() as &$ref_rl_row) {
                ...
                foreach ($dt_check_row as $row) {
                    $ref_rl_row['debit_amt'] = $ref_rl_row['debit_amt'] + $row['net_debit_amt'];
                }
            ...
            foreach ($this->bo->receivable_ledger_alloc_tran->Rows() as &$ref_tran_row) {
                ...
                $cmm = $ac->getInsertCmm();
                $detailpkid = md5($this->bo->invoice_id . ':' . $ref_tran_row['sl_no']);
                ...
                $cmm->setParamValue('pvoucher_id', $this->bo->invoice_id);
                $cmm->setParamValue('pvch_tran_id', $this->bo->invoice_id . ':' . $ref_tran_row['sl_no']);
                ...
                $cmm->setParamValue('ptran_group', 'receivable_ledger_alloc_tran');
            ...
            $cmm = new \app\cwf\vsla\data\SqlCommand();
            $cmm->setCommandText('select * from ar.sp_rl_manual_ref_post(:prl_id, :pamt_to_be_settled) ');
            $cmm->addParam('prl_id', $this->bo->rl_id);
            $cmm->addParam('pamt_to_be_settled', $this->bo->advance_amt);
            \app\cwf\vsla\data\DataConnect::exeCmm($cmm, $cn);
            
            // Update invoice table for the adv amt
            $cmm = new \app\cwf\vsla\data\SqlCommand();
            $cmm->setCommandText('select * from ar.sp_inv_manual_ref_update(:pvoucher_id, :padv_amt_tot) ');
            $cmm->addParam('pvoucher_id', $this->bo->invoice_id);
