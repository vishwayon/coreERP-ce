 ...
 * @author Kaustubh
 */
class CustomerOPBLRefEventHandler extends \app\cwf\vsla\xmlbo\EventHandlerBase {

    public function afterFetch($criteriaparam) {
        parent::afterFetch($criteriaparam);

        $this->bo->customer_receivable_ledger_temp = new \app\cwf\vsla\data\DataTable();
        $this->bo->customer_receivable_ledger_temp->cloneColumns($this->bo->customer_receivable_ledger);
        $this->fillReceivableLedgerTemp();


        $from_date = strtotime('- 1 days', strtotime(\app\cwf\vsla\security\SessionManager::getSessionVariable('year_begin')));
        $this->bo->customer_receivable_ledger->getColumn("doc_date")->default = date("Y-m-d", $from_date);
        $this->bo->setTranColDefault('customer_receivable_ledger', 'doc_date', date("Y-m-d", $from_date));

        $this->bo->customer_receivable_ledger->getColumn("fc_type_id")->default = 0;
        $this->bo->setTranColDefault('customer_receivable_ledger', 'fc_type_id', 0);
        $this->bo->customer_receivable_ledger->getColumn("exch_rate")->default = 1;
        $this->bo->setTranColDefault('customer_receivable_ledger', 'exch_rate', 1);

        $this->bo->branch_id = \app\cwf\vsla\security\SessionManager::getSessionVariable('branch_id');
    }

    private function fillReceivableLedgerTemp() {
        $rowcount = count($this->bo->customer_receivable_ledger_temp->Rows());
        for ($i = 0; $i <= $rowcount; $i++) {
            $this->bo->customer_receivable_ledger_temp->removeRow(0);
        }

        foreach ($this->bo->customer_receivable_ledger->Rows() as $row) {
            $newRow = $this->bo->customer_receivable_ledger_temp->NewRow();
            ...
            $this->bo->customer_receivable_ledger_temp->AddRow($newRow);
        }
    }
    ...
    public function onFetch($criteriaparam, $tablename) {
        ...
        if ($tablename == 'ac.rl_pl') {
            ...
            $cmm->setCommandText('Select case when exists (Select b.* from ac.rl_pl_alloc b where b.rl_pl_id = a.rl_pl_id) then false
                                        ...
                                        a.fc_type_id, a.exch_rate, a.debit_amt_fc, a.credit_amt_fc, a.debit_amt, 
            ...
            foreach ($dtTran->Rows() as $row) {
                $newRow = $this->bo->customer_receivable_ledger->NewRow();
                $newRow['is_allow_edit'] = $row['is_allow_edit'];
                $newRow['rl_pl_id'] = $row['rl_pl_id'];
                $newRow['company_id'] = \app\cwf\vsla\security\SessionManager::getSessionVariable('company_id');
                $newRow['branch_id'] = \app\cwf\vsla\security\SessionManager::getSessionVariable('branch_id');
                ...
                $newRow['en_bill_type'] = $row['en_bill_type'];
                $newRow['is_opbl'] = $row['is_opbl'];
                $this->bo->customer_receivable_ledger->AddRow($newRow);
            }
        }
    }
    ...
    public function onSave($cn, $tablename) {
        ...
        if ($tablename == 'ac.rl_pl') {
            ...
            $cmm = new \app\cwf\vsla\data\SqlCommand();
            $cmm->setCommandText('Select * from ar.sp_customer_opbl_ref_add_update(:prl_pl_id, :pcompany_id, :pbranch_id, :pvoucher_id,
                :pdoc_date, :paccount_id, :pfc_type_id, :pexch_rate, :pdebit_amt_fc, :pcredit_amt_fc, 
                :pdebit_amt, :pcredit_amt, :pnarration, :pen_bill_type)');
            $v_id = -1;

            foreach ($this->bo->customer_receivable_ledger->Rows() as &$ref_row) {
                $rl_id = $ref_row['rl_pl_id'];
                if ($ref_row['rl_pl_id'] == null || $ref_row['rl_pl_id'] == '') {
                    $rl_id = md5(\app\cwf\vsla\entity\EntityManager::getDocSeqID('OPBL', \app\cwf\vsla\security\SessionManager::getSessionVariable('branch_id'), \app\cwf\vsla\security\SessionManager::getSessionVariable('finyear'), 'ar.customer_opbl_ref', $cn, $v_id));
                }
                $cmm->addParam('prl_pl_id', $rl_id, \app\cwf\vsla\data\SqlParamType::PARAM_INOUT);
                ...
                if ($ref_row['credit_amt'] > 0 || $ref_row['credit_amt_fc'] > 0) {
            ...
            $credit_total = round(Enumerable::from($this->bo->customer_receivable_ledger->Rows())->sum('$a==>$a["credit_amt"]'), \app\cwf\vsla\Math::$amtScale);
            $debit_total = round(Enumerable::from($this->bo->customer_receivable_ledger->Rows())->sum('$a==>$a["debit_amt"]'), \app\cwf\vsla\Math::$amtScale);
