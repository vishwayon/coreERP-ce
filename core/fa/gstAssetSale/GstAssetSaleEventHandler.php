...
/**
 * Description of GstAssetSaleEventHandler
 *
 * @author Priyanka
 */
class GstAssetSaleEventHandler extends \app\cwf\vsla\xmlbo\EventHandlerBase {
    ...
    public function afterFetch($criteriaparam) {
        ...
        if ($this->bo->as_id == "" or $this->bo->as_id == "-1") {
            ...
            foreach ($criteriaparam['formData']['SelectAssetItem'] as $pltran) {
                ...
                $newRow['gtt_bt_amt'] = $pltran['sale_amt'];
                $newRow['credit_amt'] = $pltran['sale_amt'];
                $newRow['credit_amt_fc'] = $pltran['sale_amt_fc'];
                $newRow['purchase_date'] = $pltran['purchase_date'];
...
//                if($lastDepDate == NULL){
//                    $lastDepDate =  $refrow['purchase_date'];
//                }
...
//                $adWorker->CalculateDepreciation($classinst, $refrow['asset_item_id']);
//            }
...
//            }
            ...
            foreach ($this->bo->as_tran->Rows() as &$reftranrow) {
               $reftranrow['asset_name'] = \app\cwf\vsla\utils\LookupHelper::GetLookupText('../core/fa/lookups/AssetItem.xml', 'asset_name', 'asset_item_id', $reftranrow['asset_item_id']);
            }
        }
    ...
    private function setLastDepDate($asset_item_id) {
        ...
        if (count($result->Rows()) > 0) {
            if ($result->Rows()[0]['max_date'] != '') {
                $last_dep_date = $result->Rows()[0]['max_date'];
            } 
        }
