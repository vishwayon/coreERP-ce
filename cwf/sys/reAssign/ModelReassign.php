...
class ModelReassign {
    ...
    public function getData() {
        ...
        $cmmtext = "select doc_id, bo_id, edit_view, doc_name, doc_sender_comment, user_id_from, doc_sent_on::date, doc_action, 
                user_id_to, last_updated, doc_stage_id, doc_stage_id_from, branch_id, '' as from_user, user_id_to, finyear,
                '' as to_user, false as select, -1 as new_user_id_to, '' as remark, null as next_role from sys.doc_wf 
                where user_id_to <> -1 and (case when :puser_id_from <> -1 then user_id_from = :puser_id_from else :puser_id_from = -1 end) and 
                (case when :puser_id_to <> -1 then user_id_to = :puser_id_to else :puser_id_to = -1 end) and 
                (case when :pdoc_id <> '' then doc_id = :pdoc_id else :pdoc_id = '' end)  and 
                (case when :pbo_id <> '' then bo_id = :pbo_id else :pbo_id = '' end) and
                (case when :pbranch_id <> 0 then branch_id = :pbranch_id else :pbranch_id = 0 end) and 
                finyear = :pfinyear";
