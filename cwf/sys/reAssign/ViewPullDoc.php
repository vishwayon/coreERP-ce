...
$viewerurl = '?r=cwf%2Fsys%2Freassign%2Fget-pulldata';
$purl = '?r=cwf%2Fsys%2Freassign%2Fset-pulldata';
...
<div id="contentholder"  class="view-min-width view-window1">
    <div id="contents" style="padding: 5px;margin:5px;">
        <div id="collheader" class="row">
            <div class="col-md-8">
                <h3>Pull document in workflow</h3>                  
            </div>
            ...
            <div class="col-md-2" style="float: right;">
                ...
                <button class="btn btn-sm btn-default" onclick="coreWebApp.wf_reassign_u.GetData();">
                    <span class="glyphicon glyphicon-refresh" aria-hidden="true"></span>          
                </button>
                <button class="btn btn-sm btn-default" id="cmdupdatewfreq" style="display: none;"
                        onclick="coreWebApp.wf_reassign_u.SetJsonData('<?= $purl ?>', 'POST', 'preqdata');">
        ...
        <div id="preqdata" class="row" style="display: none;margin-top: 10px;margin-left: 0px;margin-right: 0px;">
            ...
            <table id="vch_tran" class="row-border hover tran"  cellspacing="0">
                ...
                <tbody data-bind="template: { name: 'preqdata-template', foreach: dt_request, afterRender: coreWebApp.wf_reassign_u.CalTS() }">
                </tbody>
            </table>            
        </div>
        ...
        <script id="preqdata-template" type="text/html">
            <tr> 
                ...
                <td style="text-align: center">
                    <input type="checkbox" data-bind="checked: select, click: coreWebApp.wf_reassign_u.wfchecked($data, $element)">
                </td>
                <td>
    ...
    <script type="text/javascript">
        ...
        //create and bind wf_reassign_u namespace
        window.coreWebApp.wf_reassign_u = {};
        (function (wf_reassign_u) {

            function getData() {
                $('#brules').html('');
                var res = $('#reassignreq').serialize();
                $.ajax({
                    url: '?r=cwf%2Fsys%2Freassign%2Fget-pulldata',
                    type: 'GET',
                    ...
                    success: function (resultdata) {
                        ...
                        if (jsonResult.jsondata.brokenrules.length > 0) {
                            ...
                            wf_reassign_u.ModelBo = ko.mapping.fromJS(jsonResult['jsondata']);
                            ...
                            ko.applyBindings(wf_reassign_u.ModelBo, $('#preqdata')[0]);
                            wf_reassign_u.ToggleUpdate();
            ...
            wf_reassign_u.SetJsonData = setJsonData;

            function docRoleData(role_id, branch_id,ctrl) {
                $.ajax({
                    url: '?r=cwf/sys/reassign/role-user',
                    type: 'GET',
            ...
            wf_reassign_u.docRoleData = docRoleData;
            ...
            wf_reassign_u.GetTimestamp = getTimestamp;

            function calTS() {
                $('[data-bind="dateValue: doc_date"]').each(function () {
                    var temp = wf_reassign_u.GetTimestamp(this);
                    $(this).attr('data-order', temp);
                });
            }
            wf_reassign_u.CalTS = calTS;

            function toggleUpdate() {
                $('#cmdupdatewfreq').hide();
                if ($("#view_type_id option:selected").text() !== 'All') {
                    if (wf_reassign_u.ModelBo.dt_request().length > 0) {
                        $('#cmdupdatewfreq').show();
                    }
                }
            }
            wf_reassign_u.ToggleUpdate = toggleUpdate;

            function wfchecked(item, ctrl) {
                if (item.select()) {
                    if (item.next_role.next_role_id() != '') {  
                        var user_combo = $(ctrl).parent().parent().children().find('#new_user_id_to');
                        wf_reassign_u.docRoleData(item.next_role.next_role_id(), item.branch_id(), user_combo);
//                        var next_roles = item.next_role.next_role_id().split(',');                      
//                        $.each(next_roles, function (index, value) {
//                            wf_reassign_u.docRoleData(value, item.branch_id(), user_combo);
//                        });
                    }
                }
            }
            wf_reassign_u.wfchecked = wfchecked;
