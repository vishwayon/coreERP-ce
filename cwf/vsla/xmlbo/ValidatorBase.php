...
abstract class ValidatorBase {
    ...
    protected function validateIDataBingingItem(design\IDataBindingItem $fld, $data, $row_id = -1) {
        ...
        switch ($fld->getType()) {
            case design\IDataBindingItem::TYPE_FIELD :
                if ($fld->isOptional || $fld->readOnly || $fld->control == design\ControlType::LABEL) {
                    break;
                }
                $fld_id = $fld->id;
                if (strpos($fld_id, ".") > 0) {
    ...
    public function docFiscalMonthClosed($val): bool {
        $cmm = new \app\cwf\vsla\data\SqlCommand();
        $cmm->setCommandText("With doc_group_tran
                                As(
                                        select x.doc_group_id 
                                        from sys.doc_group_tran x
                                        Where x.bo_id = :pbo_id
                                ),
                                month_info
                                As(
                                        Select a.month_close, (a.annex_info->>'doc_group_ids')::bigint[] doc_group_ids
                                        From sys.fiscal_month a
                                        Where month_begin <= :pdoc_date And month_end >= :pdoc_date And company_id = :pcompany_id And finyear = :pfinyear
                                )
                                select a.month_close, case when count(b.*) > 0 then true else false end doc_close
                                from month_info a
                                Left join doc_group_tran b on b.doc_group_id = Any(a.doc_group_ids)
                                Group By a.month_close");
        $i = $this->bo;
        ...
        $cmm->addParam('pbo_id', md5($this->xBo->bometa->id));
        $dt = \app\cwf\vsla\data\DataConnect::getData($cmm);
        if (count($dt->Rows()) == 1) {
            if (boolval($dt->Rows()[0]['month_close'])) {
                return true;
            }
            if (boolval($dt->Rows()[0]['doc_close'])) {
                return true;
            }
        }
        return false;
